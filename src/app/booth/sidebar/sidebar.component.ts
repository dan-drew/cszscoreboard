...
class TabDef {
  ...
  constructor({
                ...
                selectedIcon,
                classes
              }: Required<Pick<TabDefParam, 'tab' | 'icon'>> & Pick<TabDefParam, 'name' | 'selectedIcon' | 'classes'>)
  ...
  constructor({
                ...
                action,
                classes
              }: Required<Pick<TabDefParam, 'name' | 'icon' | 'action'>> & Pick<TabDefParam, 'selectedIcon' | 'classes' | 'webOnly'>)
  constructor({name, image, action}: Required<Pick<TabDefParam, 'name' | 'image' | 'action'>>)
  constructor({tab, name, icon, selectedIcon, image, action, classes, webOnly}: TabDefParam) {
    this.tab = tab
    this.name = name || tab
    this.icon = icon
...
export class SidebarComponent {
  ...
  readonly tabs: TabDef[] = [
    new TabDef({name: 'Open Live View', icon: 'pip', action: () => this.openLive(), classes: ['green'], webOnly: true}),
    ...
    new TabDef({name: 'Help', icon: 'info-circle-fill', action: () => this.help(), webOnly: true})
  ]
